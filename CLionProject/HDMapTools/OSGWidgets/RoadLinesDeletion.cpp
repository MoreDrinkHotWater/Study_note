        ...
        case (osgGA::GUIEventAdapter::MOVE) : {
            ...
            if (picker->containsIntersections()) {
                ...
                for (const auto &intersection : intersections) {
                    ...
                    std::cout << "-----------------------------" << std::endl;
                    std::cout << "RoadLineDeletion: childNode:\n name: " << childNode->getName() << "\n" << childNode->libraryName() << "\n" << childNode->className() << std::endl;
                    std::cout << "-----------------------------" << std::endl;
                    if (childNode->getName().compare(0, 10, "RoadLines_") == 0) {
                        if (outline != nullptr) {
                            std::cout << "---------------------------" << std::endl;
                            std::cout << outline->getName() << "\n" << outline->libraryName() << "\n" << outline->className() << std::endl;
                            std::cout << "getNumParents: " << outline->getNumParents() << std::endl;
                            for (int i = 0; i < outline->getNumParents(); ++i) {
                                std::cout << outline->getParent(i)->getName() << "\t" << outline->getParent(i)->libraryName() << "\t" << outline->getParent(i)->className() << std::endl;
                            }
                            std::cout << "getNumChild: " << outline->getNumChildren() << std::endl;
                            for (int i = 0; i < outline->getNumChildren(); ++i) {
                                std::cout << outline->getChild(i)->getName() << "\t" << outline->getChild(i)->libraryName() << "\t" << outline->getChild(i)->className() << std::endl;
                            }
                        }
                        if (outline == nullptr) {
                            ...
                            // 当高亮结点不为空时 给它加上一个父结点 方便下次 reset
                            if (outline->getNumParents() == 0) {
                                childNode->getParent(0)->addChild(outline);
                            }
